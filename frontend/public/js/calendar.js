...
let VanillaCalendar = (function () {
    function VanillaCalendar(options) {
            ...
            if (opts.date.getTime() <= opts.todaysDate.getTime() - 1 && !opts.pastDates) {
                ...
                if (opts.datesFilter) {
                    if (available_week_day.length) {
                        ...
                        newDayElem.setAttribute('data-calendar-status', 'active'),
                        newDayElem.setAttribute('data-index', JSON.stringify(available_date[0].index))
                    } else {
                        newDayElem.classList.add('vanilla-calendar-date--disabled')
                    }
                } else {
        ...
        const createMonth = function () {
            ...
            opts.button_next.setAttribute('data-current-date', opts.date.getFullYear() + '-' + ("0" + (opts.date.getMonth() + 1)).slice(-2));
            opts.button_prev.setAttribute('data-current-date', opts.date.getFullYear() + '-' + ("0" + (opts.date.getMonth() + 1)).slice(-2));
